...
void loop() {
  ...
  for (uint8_t col = 0; col < COLS; col++) {
    for (uint8_t row = 0; row < DUAL_ROWS; row++) {
      ...
      frameValues[index] = (float32_t) constrain(result.result_adc0 - minVals[index], 0, 255);
      frameValues[index + 127] = (float32_t) constrain(result.result_adc1 - minVals[index + DUAL_ROW_FRAME], 0, 255);
  ...
  for (uint8_t i = 0; i < ROW_FRAME; i++) { // 16_ROWS x 16_COLC
    Serial.printf(F("%d"), (uint8_t) frameValues[i]);
    delay(10);
  }
  Serial.printf(F("\n DEBUG_ADC_INPUT!"));
  delay(1000);
...
void calib() {
  ...
  for (uint8_t col = 0; col < COLS; col++) {
    for (uint8_t row = 0; row < DUAL_ROWS; row++) {
      ...
      minVals[index + DUAL_ROW_FRAME] = (uint8_t) constrain(result.result_adc1 - minVals[index + DUAL_ROW_FRAME], 0, 255);
