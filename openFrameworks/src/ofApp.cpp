...
void ofApp::update() {
    ...
    if (newFrame) {
        ...
        for (int i=0; i<contourFinder.blobs.size(); i++) {
            ...
            if (DEBUG_PRINT) {
                ofLog() << "BlobID : "<< " posX : " << int(posX) << " posY : " << int(posY) << " posZ : " << posZ << endl;
            }
            centroid c = {};
        ...
        for(int i = 0 ; i < centroidsToAdd.size() ; i++ ) {
          ...
          for(int i = 0 ; i < centroids.size() ; i++) {
              if( ! centroids[i].isDead) {
                  ...
                  if (DEBUG_PRINT) {
                      cout << "BlobID : "<<centroids[i].UID<< " posX : " << centroids[i].position.x << " posY : " << centroids[i].position.y << " posZ : " << centroids[i].pressure << endl;
                  }
